/**
 * Recherche d'une finition en français pour le simulateur : les noms du
 * catalogue Cover Styl' sont en anglais (« Walnut Ash », « Black Mat »), le
 * visiteur tape « noyer » ou « noir mat ». Chaque mot tapé doit commencer un
 * mot du nom, de la référence, de la famille ou des étiquettes — ou de sa
 * traduction. Sans accents ; un début de mot suffit (« noy » → noyer), mais
 * « vert » ne trouve pas « travertine ».
 * (Le CRM a la même recherche pour son simulateur : src/lib/simulateur/recherche-teintes.ts.)
 */
...
export function correspondRecherche(r: { id: string; nom: string; famille: string; categorie?: string; finition?: string; tags: string[] }, recherche: string): boolean {
  const mots = sansAccents(recherche).split(/[^a-z0-9]+/).filter(Boolean);
  if (mots.length === 0) return true;
  const texte = sansAccents(`${r.id} ${r.nom} ${r.famille} ${r.categorie ?? ""} ${r.finition ?? ""} ${r.tags.join(" ")}`);
